...
import { NavItem, NavMainItem } from './nav-item';
...
export function NavMainList() {
  ...
  const activeMainMenu = useMainRoute((state: any) => state.route)
    ...
    return routes.map((data) => {
      return (
        <NavMainItem
          ...
          caption={data.meta.label}
          active={activeMainMenu == data.name}
          onClick={() => updateRoute(data.name)}
        />
      )
    })
