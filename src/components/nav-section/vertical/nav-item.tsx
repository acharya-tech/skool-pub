...
export const NavMainItem = forwardRef<HTMLButtonElement, Pick<NavItemProps, 'onClick' | 'path' | 'caption' | 'icon' | 'title' | 'active' | 'disabled'>>((props, ref) => {
  const {
    path,
    icon,
    title,
  ...
  return (
    <ItemRoot
      ...
      {...ownerState}
      {...navItem.baseProps}
      className={mergeClasses([navSectionClasses.item.root, ''], {
        [navSectionClasses.state.open]: active,
        [navSectionClasses.state.active]: active,
        [navSectionClasses.state.disabled]: disabled,
      })}
      {...other}
      sx={{
        ...
        ...(active && {
           bgcolor: 'var(--layout-main-nav-bg-active-color)',
           color: 'var(--layout-main-nav-text-active-color)',
        })
      }}
    >
      {icon && (
        <ItemIcon {...ownerState} className={navSectionClasses.item.icon}>
          {navItem.renderIcon}
        </ItemIcon>
      )}
