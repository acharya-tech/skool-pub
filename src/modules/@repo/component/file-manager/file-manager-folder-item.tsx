...
export function FileManagerFolderItem({
  ...
  const favorite = useBoolean(Boolean(folder.starred));
  ...
  const [folderName, setFolderName] = useState(folder.label);
  ...
  const renderAvatar = () => (
    ...
    >
      {folder.shared?.map((person) => (
        <Avatar key={person.id} alt={person.name} src={person.image?.url} />
      ))}
    </AvatarGroup>
  );
