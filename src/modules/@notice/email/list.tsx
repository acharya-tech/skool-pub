...
import { NOTICE_EMAIL_LIST } from "../constant/local.urls";
...
import { NOTICE_EMAIL_GROUP_URL } from "../constant/server.urls";
import { INoticeEmail } from "../interface";
export const EmailListTable = ({ search }: TableListProp) => {
    const t = useTranslate(LANG_NOTICE, "email");
    const { show } = useNav(NOTICE_EMAIL_LIST);
    ...
    const columns = useMemo<GridColDef<INoticeEmail>[]>(
        () => [
            {
                field: "title",
    ...
    const { dataGridProps, setFilters } = useDataGrid<INoticeEmail, HttpError>({
        resource: NOTICE_EMAIL_GROUP_URL
    });
