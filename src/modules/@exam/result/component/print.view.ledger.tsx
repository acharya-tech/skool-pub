...
import { Markledger } from "@components/editorItem/Markledger";
...
const Ledger = ({ results, template, version, pageNumber, setRefs }: LedgerProps) => {
    ...
    const fillers: Record<string, string | number> = {
        result_date: dayjs(routine.result_date).format("YYYY-MM-DD"),
                        ...
                        if (item.type === 'ledgerTable') {
                            if (item.tableId === 'neb_xii_gradeledger') {
                                return <GradeLedger
                                    ...
                                    rowSize={item.rows}
                                />
                            } else if (item.tableId === 'neb_xii_markledger') {
                                return <Markledger
                                    el={item}
                                    subjects={routine.esubjects}
                                    results={results}
