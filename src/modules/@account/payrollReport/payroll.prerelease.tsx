...
import { IPayrollReleaseResponse } from "../interface";
import { ACCOUNT_PAYROLL_PREPOST_SUMMARY_ID } from "../constant/constant";
...
export const PayrollPrePostTable = ({ search = "" }: TableListProp) => {
  const t = useTranslate(LANG_ACCOUNT, "report");

  const { data } = useOne<IPayrollReleaseResponse>({
    resource: ACCOUNT_REPORT_URL,
    id: ACCOUNT_PAYROLL_PREPOST_SUMMARY_ID,
  })

  const { ledgers = [], reports = [], employees = [] } = data?.data ?? {};
