  ...
  useEffect(() => {
    ...
    if (dataParam) {
      try {
        ...
        // Validate if the parsed data is an array of objects with the expected structure
        if (
    ...
    <Box sx={{ padding: 2 }}>
      ...
      <Box id="print-content" className="print-container">
        <Grid container direction="row" spacing={2}>
          {data.length > 0 ? (
            data.map((item, index) => (
              <Grid size={3} key={index} className="print-item">
                <Box
                  sx={{
                    display: "flex",
                ...
                >
                  ...
                  >
                    ...
                    {/* Barcode for Accession No */}
                    <Box sx={{ margin: "" }}>
                      <Barcode value={item.accession_no} />
                    </Box>
                  </Box>
                </Box>
              </Grid>
