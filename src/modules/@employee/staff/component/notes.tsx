...
import { EMPLOYEE_NOTE_URL } from '../../constant';
import { IEmployeeNote } from '../../interface';
...
const NoteInput = ({ id }: any) => {
    ...
    const { mutate } = useCreate({
        resource: EMPLOYEE_NOTE_URL
    })
    const [confirm, confirmEle] = useConfirm({
        onConfirm: (data) => {
            mutate({ values: { ...data, rating: parseInt(data.rating), user_staff_id: id } }, {
...
const EmployeeNotes = ({ id }: EmployeeViewProps) => {

    const { data, isLoading } = useList<IEmployeeNote>({
        meta: { customQuery: { user_staff_id: id } },
        resource: EMPLOYEE_NOTE_URL,
    });
    const [
        {
