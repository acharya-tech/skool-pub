...
import { STUDENT_DASHBOARD_CERTIFICATE_PIE_ID } from '@student/constant/constant';
...
export function GroupPie({ title, chart, subheader, sx, ...other }: Props) {
  ...
  const { data: chartDataRaw } = useOne<IPieReportDto>({
    ...
    id: EMPLOYEE_DASHBOARD_GROUP_PIE_ID
  })
  ...
  return (
    <Card sx={sx} {...other}>
      <CardHeader title={title ?? t("labels.groupPie")} subheader={subheader} />
      <Chart
        type="pie"
        series={chartSeries}
        options={chartOptions}
