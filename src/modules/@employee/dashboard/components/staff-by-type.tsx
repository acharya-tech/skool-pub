...
import { EMPLOYEE_DASHBOARD_STAFF_TYPE_ID } from '@employee/constant/constant';
...
export function StaffByType({ title, subheader, chart, sx, ...other }: Props) {
  ...
  const { data: chartData } = useOne<IDataActivityReportDto>({
    ...
    id: EMPLOYEE_DASHBOARD_STAFF_TYPE_ID
  })
  ...
  return (
    <Card sx={sx} {...other}>
      <CardHeader
        title={title ?? t("labels.staffByType")}
        subheader={subheader}
      />
      <Chart
        type="bar"
